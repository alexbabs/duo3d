...
int main(int argc, char* argv[])
{
	...
	//use timer to save 3dmap every 10 sec
	long int start_time;
	struct timespec gettime_now;
	clock_gettime(CLOCK_REALTIME, &gettime_now);
	start_time = gettime_now.tv_sec;		//Set start time value
	...
	while((cvWaitKey(1) & 0xff) != 27)
	{
						  ...
						  (float*)disparity.data, (PDense3DDepth)depth3d.data))
		{
			...
			//check if time has elepsed to save 3dmap
			clock_gettime(CLOCK_REALTIME, &gettime_now);
			if ((gettime_now.tv_sec - start_time)>20)
			{
				start_time = gettime_now.tv_sec;
				
				std::wstringstream fileName;
				fileName << L"Dense3DPointCloud" << rawtime << L".ply";	//set file name with time stamp
				Dense3DSavePLY (dense3d, fileName.str().c_str())
				fileName << L"DepthMap" << rawtime << L".jpg";	//set file name with time stamp
				imwrite(fileName.str().c_str(), disp8)	//save gray scale depth map image
		}
		// Display images
